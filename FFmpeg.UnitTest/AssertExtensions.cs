...
namespace HanumanInstitute.FFmpeg.UnitTests
{
    public static class AssertExtensions
    {
        ...
        public static void ContainsOrNot(this string actualValue, string expectedSubstring, bool contains)
        {
            if (contains)
            {
                Assert.Contains(expectedSubstring, actualValue, StringComparison.InvariantCulture);
            }
            else
            {
                Assert.DoesNotContain(expectedSubstring, actualValue, StringComparison.InvariantCulture);
            }
        }
    }
}
